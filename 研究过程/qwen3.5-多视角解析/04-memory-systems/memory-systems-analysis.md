...
| 策略 | 触发条件 | 压缩方式 | 特点 |
|------|----------|----------|------|
| **Snip** | 接近阈值 | 直接删除旧消息 | 快速，不生成摘要 |
| **Microcompact** | 小范围超限 | 小范围压缩 | 缓存编辑友好 |
| **Autocompact** | 超过阈值 | LLM 生成摘要 | 保留语义信息 |
| **ReactiveCompact** | API 拒绝 | 被动压缩 | 最后防线 |
...
## D.5 Harness Engineering 评价

| 维度 | 评分 | 说明 |
|------|------|------|
| 可扩展性 | ⭐⭐⭐⭐ | 记忆存储后端抽象 | 压缩策略可插拔 |
| 安全边界 | ⭐⭐⭐⭐ | 敏感信息过滤 | paste store 隔离 |
| 可观察性 | ⭐⭐⭐⭐ | 上下文使用监控 | token 计数追踪 |
| 性能损耗 | ⭐⭐⭐ | 记忆检索开销 | 压缩 LLM 调用成本 |
...
class MemoryStore:
    ...
    def compress(
        self, entries: List[str], compressor: Callable[[List[str]], str]
    ) -> CompressedMemory:
        """压缩记忆"""
        contents = [
...
class ContextManager:
    ...
    def _compress_old_messages(
        self, compressor: Callable[[List[str]], str] = None
    ):
        """压缩旧消息"""
...
1. **压缩质量不一致**: snip/micro/autocompact 产生的摘要质量参差不齐
2. **记忆检索效率低**: 线性搜索在大规模记忆中效率低下
